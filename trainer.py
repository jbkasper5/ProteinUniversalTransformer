# from dataloader import SCN_DataLoader
...
# # REMEMBER TO TYPE [wandb enabled] IN THE COMMAND LINE
# wandb.init(project = "SummerResearch2022", entity = "jbkasper5")
# wandb_logger = WandbLogger()


# scn_data = SCN_DataLoader()
...
#     heads = 8,
#     dropout = 0.1,
#     max_length = scn_data.max_len,
# )

# trainer = Trainer()
# trainer.fit(model, scn_data)
# trainer.test(model, scn_data)
...
from transformer import Transformer
from scn_dataloader import LitSCNDataModule
...
def create_parser():
    ...
    data_args.add_argument("--scn_dir",
                           type=str,
                           default="/Users/jakekasper/Desktop/Research/sidechainnet_data",
...
def main():
    ...
    data = scn.load("debug", with_pytorch = 'dataloaders', batch_size = 12, dynamic_batching = True)
    ...
    wandb_logger, checkpoint_dir = init_wandb(use_cluster=None,
                                              project="SummerResearch2022",
                                              entity="jbkasper5",
                                              model=model,
                                              dict_args=dict_args)
    dict_args.update(
